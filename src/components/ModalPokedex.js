...
class ModalPokedex extends React.Component {
  ...
  render() {
    ...
    const attacks =
      pokemon && pokemon.moves.slice(0, 4).map((attack) => attack.move.name);
    const weakness = ["Fire", "Flight", "Psy", "Ice"];
    const firstType = pokemon && pokemon.types[0].type.name;
    const scdType =
      pokemon && pokemon.types[1] ? pokemon.types[1].type.name : "";
    return (
      <div className="ModalPokedex" style={{ display: isDisplay }}>
        <div className="modal">
          <div className="modal-content">
            <span onClick={this.props.method} className="close">
              &times;
            </span>
            <h3>
              {pokemon.name}
            ...
            <Link
              to={
                this.props.selectPlayer1.length > 0
                  ? "/new-game-5"
                  : "/new-game-1"
              }
            >
            ...
            <div
              style={
                !this.props.modalButton ? { height: "50px" } : { height: 0 }
              }
            ></div>
              ...
              <div>
                ...
                <div className="stats-container">
                  <h4>
                    Type :{" "}
                    <span>
                      {firstType} {scdType && scdType}
                    </span>
                  </h4>
                  <div className="simple-data">
                    <h4>
                      Size : <span>{pokemon.height / 10} m</span>
                    </h4>
                    <h4>
